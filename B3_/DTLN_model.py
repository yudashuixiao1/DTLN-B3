...
class audio_generator():
    ...
    def create_generator(self):
        ...
        for file in self.file_names:
            ...
            noisy = noisy + 1e-8
            speech, fs_2 = sf.read(os.path.join(self.path_to_s1, file)) #, dtype='float32'
            speech = speech + 1e-8
...
class DTLN_model(Signal_Pro):
    ...
    def __init__(self):
        ...
        self.L = (16000 * self.len_samples - self.block_len) // self.block_shift + 1
        self.filter_size = [32,32,32,64,128] #[1, 16, 32, 64, 64]
        self.kernel_size = [[1,5],[1,3],[1,3],[1,3],[1,3]]
        self.strides = [[1,2],[1,2],[1,2],[1,1],[1,1]]
        self.encoder_padding = [[0,2],[0,1],[0,1],[1,1],[1,1]]
        self.decoder_padding = ['same', 'same', 'same', 'same', 'valid']
        self.output_cut_off = 2
        self.intra_hidden_size = 128
        self.inter_hidden_size = 128
        self.N_DPNN = 2
        self.use_CuDNNGRU = False
    ...
    def stftLayer_s(self, x, mode ='mag_pha'):
        '''
        Method for an STFT helper layer used with a Lambda layer. The layer
        calculates the STFT on the last dimension and returns the magnitude and
        phase of the STFT.
    ...
    def ifftLayer(self, x):
        ...
        s1_ifft = tf.signal.irfft(s1_stft)
        # if win:
        #     s1 = s1_ifft * self.win
        # else:
        #     s1 = s1_ifft
    ...
    def build_DTLN_model_B3(self, norm_stft=False):

        # input layer for time signal
        time_dat = Input(batch_shape=(None, None))
        # calculate STFT
        mag,angle = Lambda(self.stftLayer)(time_dat)
        print('shape of mag', np.shape(mag))
        # normalizing log magnitude stfts to get more robust against level variations
        if norm_stft:
            mag_norm = InstantLayerNormalization()(tf.math.log(mag + 1e-7))
        else:
        ...
        #encoded_frames_norm = InstantLayerNormalization()(encoded_frames)
        # predict mask based on the normalized feature frames
        mask_2 = self.seperation_kernel(self.numLayer, self.encoder_size+1, estimated_mag)
        # multiply encoded frames with the mask
        estimated = Multiply()([estimated_mag, mask_2]) 
        # decode the frames back to time domain

        #print('angle2 shape', np.shape(angle2))
        frames2 = Lambda(self.ifftLayer)([estimated ,angle])
        # create waveform with overlap and add procedure
        estimated_sig = Lambda(self.overlapAddLayer)(frames2)
